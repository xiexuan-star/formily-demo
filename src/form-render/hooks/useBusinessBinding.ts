...
export function useBusinessBinding() {
  ...
  const handlerMap = new Map<FIELD_BUSINESS_TYPE, (formModel: Form, collector: BusinessCollector, value: unknown) => void>([
    [FIELD_BUSINESS_TYPE.ID_CARD, handlerIdCardType]
  ]);

  function trigger(collector: BusinessCollector, formModel: Form, fieldName: string, value: unknown) {
