...
import { arrayed, transformDateFormat } from '../utils';
...
export function useFieldList2Schema(collector: BusinessCollector) {
  ...
  const createSliderSchema: SchemaCreator = item => {
    const schema = createStandardSchema(item);
    Object.assign(schema['x-component-props'], {
  ...
  const createWidgetCombinationSchema: SchemaCreator = item => {
    const suffixList: FieldItem[] = arrayed(item.suffixConfig);
